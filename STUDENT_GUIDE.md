...
本指南适用于 Windows（PowerShell）或 macOS（终端）+ Codex。教师会通过私密渠道分别发送给你：

- 课程安装脚本：Windows 为 `bootstrap.ps1`，macOS 为 `bootstrap.sh`；
- 作业代码；
- 你的个人提交令牌（`submit_token`）。

令牌相当于提交密码：不要截图、转发或写入 Git 仓库。每位同学各用各的令牌。
...
```text
请按 STUDENT_GUIDE.md 协助我安装 Vibe 作业提交插件。安装脚本是老师发来的 bootstrap 文件；请先检查脚本中没有 TODO 占位符，再引导我运行它。不要要求我把 submit_token 发到对话中：我只会在本机终端出现 submit_token 提示时自行输入。安装检查成功后，如本机有 codex 命令，请运行 `codex plugin add vibe-submit@vibe-course` 安装插件；否则指导我重启 Codex，并在 Vibe Course Marketplace 中安装“Vibe 作业提交”插件。
```

Codex 可能会请求你批准安装 `uv` 或运行脚本；仅在确认脚本来自老师、服务器地址正确时批准。`submit_token` 只应输入到本机终端的交互提示中，**不要粘贴到 Codex 对话、截图或作业文件**。
...
3. 第一次执行时会自动安装 `uv`（Python 工具）并登记课程 Marketplace。服务器地址已由老师写入脚本；按提示只输入你的学号和个人令牌，且不要修改服务器地址。
...
3. 脚本会在你的用户目录安装 `uv`，不需要管理员密码。服务器地址已由老师写入脚本；按提示只输入你的学号和个人令牌，且不要修改服务器地址。
