...
def _client_version() -> str:
    """Return the installed package version, falling back to the module value."""
    try:
...
def _cmd_retry(args: argparse.Namespace) -> int:
    ...
    # Preserve the original server URL stored with the outbox entry.
    try:
        outbox_cfg = get_outbox_config(args.outbox_id)
    except Exception:
        outbox_cfg = cfg
    cfg = Config(
        server_url=outbox_cfg.server_url or cfg.server_url,
        student_no=cfg.student_no,
